...
class _LoginPageState extends State<LoginPage> with ValidationMixin {
  ...
  Future<void> _login() async {
    Log.d('LoginPage: _login() called.'); // Added log
    if (!(_formKey.currentState?.validate() ?? false)) {
      Log.d('LoginPage: Form validation failed.'); // Added log
      return;
    }
    Helpers.hideKeyboard(context);
    Log.d('LoginPage: Dispatching LoggedIn event.'); // Added log
  ...
  void _navigateToHome() {
    // Check the global auth state to avoid race conditions
    final isAuthenticated = AuthStateProvider.instance.isAuthenticated;
    final currentUser = AuthStateProvider.instance.currentUser;

    if (isAuthenticated && currentUser != null) {
      // Determine the home path based on user type from the global provider
      final homePath = currentUser.userType == UserType.driver
          ? RouteNames.driverDashboardPath
          : (currentUser.userType == UserType.admin
          ? RouteNames.passengerHomePath // Admin goes to passenger home for now
          : RouteNames.passengerHomePath);

      Log.i("LoginPage: Direct navigation to $homePath for user type: ${currentUser.userType}");
      context.go(homePath);
    } else {
      Log.w("LoginPage: Cannot navigate - global auth state is not authenticated");
    }
  }
    ...
    return Scaffold(
      ...
      body: BlocListener<AuthBloc, AuthState>(
        listener: (context, state) {
          Log.d('LoginPage Listener: Received AuthBloc state: ${state.runtimeType}');

          final isLoading = state is AuthLoading;
          if (_isLoading != isLoading) {
            Log.d('LoginPage Listener: Setting _isLoading to $isLoading.');
          ...
          if (state is AuthFailure) {
            Log.w('LoginPage Listener: Received AuthFailure state: ${state.message}');
            WidgetsBinding.instance.addPostFrameCallback((_) {
              if (mounted) Helpers.showSnackBar(context, message: state.message, isError: true);
            });
          }
          else if (state is AuthAuthenticated) {
            Log.i("LoginPage Listener: AuthAuthenticated received for user type: ${state.user.userType}");
            ...
            WidgetsBinding.instance.addPostFrameCallback((_) {
              if (mounted) {
                ...
                // Give the router a chance to handle it first
                Future.delayed(const Duration(milliseconds: 300), () {
                  // Check if we're still on the login page
                  if (mounted && ModalRoute.of(context)?.settings.name?.contains("login") == true) {
                    Log.w("LoginPage: Router redirect failed - using direct navigation fallback");
                    // Use direct navigation method that checks the global auth state
                    _navigateToHome();
                  } else {
                    Log.d("LoginPage: Router handled redirection successfully or widget unmounted");
                  }
                });
              }
            });
            ...
            Log.d("LoginPage Listener: Received AuthUnauthenticated state.");
          }
          else {
            Log.d("LoginPage Listener: Received state: ${state.runtimeType}");
          }
        },
        child: Stack(
          children: [
...
//       body: BlocListener<AuthBloc, AuthState>(
//         listener: (context, state) {
//           Log.d('LoginPage Listener: Received AuthBloc state: ${state.runtimeType}'); // Added detailed log
//
//           final isLoading = state is AuthLoading;
//           if (_isLoading != isLoading) {
//             Log.d('LoginPage Listener: Setting _isLoading to $isLoading.'); // Added log
//             WidgetsBinding.instance.addPostFrameCallback((_) {
//               if (mounted) setState(() { _isLoading = isLoading; });
//             });
//           }
//
//           if (state is AuthFailure) {
//             Log.w('LoginPage Listener: Received AuthFailure state: ${state.message}'); // Added log
//             WidgetsBinding.instance.addPostFrameCallback((_) {
//               if (mounted) Helpers.showSnackBar(context, message: state.message, isError: true);
//             });
//           }
...
//           else if (state is AuthAuthenticated) {
//             Log.i("LoginPage Listener: AuthAuthenticated received. Preparing to navigate home."); // Added log
//             WidgetsBinding.instance.addPostFrameCallback((_) {
//               if (mounted) {
//                 // ADDED DELAY: Introduce a small delay
//                 Future.delayed(const Duration(milliseconds: 500), () { // Adjusted delay
//                   if (mounted) { // Check mounted again after delay
//                     Log.i("LoginPage Listener: Widget mounted after delay, navigating home."); // Added log
...
//                     Log.i("LoginPage Listener: Navigating to home path: $homePath"); // Added log
//                     // Use context.go to replace the entire stack with the home route
//                     context.go(homePath);
//                   } else {
//                     Log.w("LoginPage Listener: Widget not mounted after delay, cannot navigate."); // Added log
//                   }
//                 });
//               } else {
//                 Log.w("LoginPage Listener: Widget not mounted, cannot navigate immediately."); // Added log
//               }
//             });
//           } else if (state is AuthUnauthenticated) {
//             Log.d("LoginPage Listener: Received AuthUnauthenticated state."); // Added log
//             // No specific action needed here, router redirect handles moving to login if on protected route
//           } else {
//             Log.d("LoginPage Listener: Received unexpected state: ${state.runtimeType}"); // Added log for other states
//           }
//         },
//         child: Stack(
//           children: [
...
// //             WidgetsBinding.instance.addPostFrameCallback((_) {
// //               if (mounted) {
// //                 Log.i("LoginPage Listener: Widget mounted, navigating home."); // Added log
// //                 // Determine home route based on user type
// //                 final homePath = state.user.isDriver ? RouteNames.driverDashboardPath : RouteNames.passengerHomePath;
// //                 Log.i("LoginPage Listener: Navigating to home path: $homePath"); // Added log
// //                 // Use context.go to replace the entire stack with the home route
// //                 context.go(homePath);
// //               } else {
// //                 Log.w("LoginPage Listener: Widget not mounted, cannot navigate."); // Added log
// //               }
// //             });
// //           } else if (state is AuthUnauthenticated) {
// //             Log.d("LoginPage Listener: Received AuthUnauthenticated state."); // Added log
