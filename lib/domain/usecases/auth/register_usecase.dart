...
class RegisterUseCase implements UseCase<UserEntity, RegisterParams> {
  ...
  Future<Either<Failure, UserEntity>> call(RegisterParams params) async {
    Log.d('RegisterUseCase: Executing with params.'); // Added log
    // Log the parameters received by the use case
    Log.d('RegisterUseCase: Received params - email: ${params.email}, password: ${params.password.isNotEmpty ? "[REDACTED]" : "EMPTY"}, confirmPassword: ${params.confirmPassword.isNotEmpty ? "[REDACTED]" : "EMPTY"}, firstName: ${params.firstName}, lastName: ${params.lastName}, userType: ${params.userType.name}'); // Added detailed logging for params
    Log.d('RegisterUseCase: Received optional params - phoneNumber: ${params.phoneNumber}, language: ${params.language?.name}'); // Added logging for optional params
    Log.d('RegisterUseCase: Received driver params - idNumber: ${params.idNumber}, idPhoto: ${params.idPhoto != null ? "Present" : "Null"}, licenseNumber: ${params.licenseNumber}, licensePhoto: ${params.licensePhoto != null ? "Present" : "Null"}'); // Added logging for driver params


    // Potential complex validation (e.g., password strength beyond basic checks) could go here.

    Log.d('RegisterUseCase: Calling repository.register().'); // Added log before calling repository
    ...
    Log.d('RegisterUseCase: Received result from repository.register().'); // Added log after calling repository

    return result.fold(
          (failure) {
        Log.e('RegisterUseCase: Repository.register() failed.'); // Added error log
        // Log the failure details received from the repository
        Log.e('RegisterUseCase: Failure details from repository - Type: ${failure.runtimeType}, Message: ${failure.message}'); // Added detailed failure log
        return Left(failure); // Pass the failure along
      },
          (userEntity) {
        Log.i('RegisterUseCase: Repository.register() successful. User ID: ${userEntity.id}'); // Added success log
        return Right(userEntity); // Pass the user entity along
      },
    );
  }
}
